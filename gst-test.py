...
/* ICONE OEIL (password) → claire */
.stTextInput button svg { color: #BBBBBB !important; fill: #BBBBBB !important; opacity: 0.7 !important; }
.stTextInput button:hover svg { color: #888888 !important; fill: #888888 !important; }
...
# ─── Session — persistance après refresh ──────────────────────────────────────
# NE PAS écraser les valeurs existantes → la session survit au refresh
for k, v in [("authenticated", False), ("username", ""), ("role", ""), ("lots_autorises", []), ("auth_page", "login")]:
    if k not in st.session_state:
        st.session_state[k] = v
...
def page_login():
    ...
    with r:
        ...
        if st.button("Se connecter", key="btn_login", use_container_width=True):
            ...
            st.session_state.role = str(user["role"])
            lots_raw = str(user.get("lots_autorises",""))
            st.session_state.lots_autorises = [l.strip() for l in lots_raw.split(",") if l.strip()]
            st.rerun()
        st.markdown('<div class="auth-divider">ou</div>', unsafe_allow_html=True)
        if st.button("Créer un compte", key="btn_go_register", use_container_width=True):
            st.session_state.auth_page = "register"; st.rerun()
...
with dcol:
    if st.button("Déconnexion", key="btn_logout"):
        for k in ["authenticated","username","role","lots_autorises"]: st.session_state.pop(k, None)
        st.session_state.auth_page = "login"; st.rerun()
...
if is_admin:
    ...
    with tab1:
        ...
        with c1:
            ...
            if personne_input:
                sug_p = [p for p in personnes_existantes if personne_input.upper() in p.upper()
                         and personne_input.upper() != p.upper()][:4]
                if sug_p:
                    sp_cols = st.columns(len(sug_p))
                    for i, sp in enumerate(sug_p):
                        with sp_cols[i]:
                            if st.button(sp, key=f"sp_{sp}_{i}"):
                                st.session_state["personne_input"] = sp; st.rerun()
            type_trans = st.selectbox("Type de transaction", ["ACHAT","VENTE","DÉPENSE"])
        ...
        with c2:
            ...
            if lot_input:
                sug_l = [l for l in lots_existants if lot_input.upper() in l.upper()
                         and lot_input.upper() != l.upper()][:4]
                if sug_l:
                    sl_cols = st.columns(len(sug_l))
                    for i, sl_item in enumerate(sug_l):
                        with sl_cols[i]:
                            if st.button(sl_item, key=f"sl_{sl_item}_{i}"):
                                st.session_state["lot_input"] = sl_item; st.rerun()
            description = st.text_input("Description")
            montant     = st.number_input("Montant (MAD)", min_value=0.0, step=0.01)
