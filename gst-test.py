...
/* ── Finance redesign ── */
.fin-header{display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:2rem;padding-bottom:1.2rem;border-bottom:1px solid #E0DDD5}
.fin-header-left{}
.fin-title{font-family:'DM Serif Display',serif;font-size:2rem;color:#1C1C1C;line-height:1;margin-bottom:0.3rem}
.fin-subtitle{font-size:0.72rem;color:#AAA;letter-spacing:0.14em;text-transform:uppercase;font-weight:500}
.fin-date{font-size:0.72rem;color:#BBB;letter-spacing:0.06em}
.fin-kpi-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:#E0DDD5;border-radius:12px;overflow:hidden;margin-bottom:2rem}
.fin-kpi-cell{background:#FFFFFF;padding:1.4rem 1.6rem;position:relative}
.fin-kpi-cell:first-child{border-radius:12px 0 0 12px}
.fin-kpi-cell:last-child{border-radius:0 12px 12px 0}
.fin-kpi-label{font-size:0.65rem;font-weight:600;letter-spacing:0.14em;text-transform:uppercase;color:#BBBBBB;margin-bottom:0.8rem;display:flex;align-items:center;gap:0.4rem}
.fin-kpi-label-dot{width:5px;height:5px;border-radius:50%;flex-shrink:0}
.fin-kpi-value{font-family:'DM Serif Display',serif;font-size:1.65rem;line-height:1;letter-spacing:-0.02em;margin-bottom:0.25rem}
.fin-kpi-sub{font-size:0.68rem;color:#BBBBBB;letter-spacing:0.04em}
.fin-kpi-accent{position:absolute;top:0;left:0;width:3px;height:100%}
.fin-nav{display:flex;gap:0;border:1px solid #E0DDD5;border-radius:8px;overflow:hidden;margin-bottom:1.8rem;width:fit-content;background:#FAFAF8}
.fin-nav-item{font-size:0.75rem;font-weight:500;letter-spacing:0.06em;text-transform:uppercase;color:#999;padding:0.55rem 1.3rem;cursor:pointer;border-right:1px solid #E0DDD5;transition:all 0.15s ease;white-space:nowrap}
.fin-nav-item:last-child{border-right:none}
.fin-nav-item.active{background:#1C1C1C;color:#F7F6F2}
.fin-nav-item:hover:not(.active){background:#F0EDE5;color:#555}
.fin-section{background:#FFFFFF;border:1px solid #E8E5DE;border-radius:12px;overflow:hidden;margin-bottom:1.2rem}
.fin-section-head{display:flex;align-items:center;justify-content:space-between;padding:1rem 1.4rem;border-bottom:1px solid #F0EDE5;cursor:pointer;user-select:none}
.fin-section-head-left{display:flex;align-items:center;gap:0.7rem}
.fin-section-icon{width:28px;height:28px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:0.8rem;flex-shrink:0}
.fin-section-label{font-size:0.8rem;font-weight:600;letter-spacing:0.04em;color:#1C1C1C}
.fin-section-meta{font-size:0.7rem;color:#BBB}
.fin-section-arrow{font-size:0.65rem;color:#CCC;transition:transform 0.2s}
.fin-section-body{padding:1.4rem}
.fin-row-stat{display:flex;gap:0.7rem;flex-wrap:wrap;margin-bottom:1.2rem}
.fin-stat-pill{background:#F7F6F2;border:1px solid #E8E5DE;border-radius:8px;padding:0.6rem 1rem;flex:1;min-width:120px}
.fin-stat-pill-label{font-size:0.62rem;font-weight:600;text-transform:uppercase;letter-spacing:0.1em;color:#BBB;margin-bottom:0.3rem}
.fin-stat-pill-value{font-family:'DM Serif Display',serif;font-size:1.1rem;color:#1C1C1C;line-height:1}
.fin-form-wrap{background:#F7F6F2;border-radius:8px;padding:1.2rem 1.4rem;margin-bottom:1.2rem}
.fin-form-title{font-size:0.7rem;font-weight:600;letter-spacing:0.12em;text-transform:uppercase;color:#999;margin-bottom:1rem;display:flex;align-items:center;gap:0.5rem}
.fin-divider{height:1px;background:#F0EDE5;margin:1.2rem 0}
.fin-table-label{font-size:0.7rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#AAA;margin-bottom:0.6rem;margin-top:0.2rem}
.fin-action-row{background:#F7F6F2;border:1px solid #E8E5DE;border-radius:8px;padding:1rem 1.2rem;margin-top:1rem}
.fin-action-title{font-size:0.7rem;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:#999;margin-bottom:0.7rem}
...
@media screen and (max-width:768px){
  ...
  .fin-kpi-cell:first-child{border-radius:12px 12px 0 0 !important}
  .fin-kpi-cell:last-child{border-radius:0 0 12px 12px !important}
}
...
def render_finance_tab(lots_list):
    ...
    sol_color = "#2D7A3A" if kpi_sol >= 0 else "#B03A2E"
    sol_sign  = "+" if kpi_sol >= 0 else ""
    ...
    <div class="fin-header">
      <div class="fin-header-left">
        ...
        <div class="fin-subtitle">Vue consolidée · Dettes · Caisse · Encaissements</div>
      </div>
      <div class="fin-date">{today_str}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="fin-kpi-grid">
      <div class="fin-kpi-cell">
        <div class="fin-kpi-accent" style="background:#B03A2E"></div>
        <div class="fin-kpi-label"><span class="fin-kpi-label-dot" style="background:#B03A2E"></span>Dette financière</div>
        <div class="fin-kpi-value" style="color:#B03A2E">{kpi_df:,.0f}</div>
        <div class="fin-kpi-sub">MAD restant</div>
      </div>
      <div class="fin-kpi-cell">
        <div class="fin-kpi-accent" style="background:#C0864A"></div>
        <div class="fin-kpi-label"><span class="fin-kpi-label-dot" style="background:#C0864A"></span>Dette fournisseur</div>
        <div class="fin-kpi-value" style="color:#7A4100">{kpi_dfo:,.0f}</div>
        <div class="fin-kpi-sub">MAD restant</div>
      </div>
      <div class="fin-kpi-cell">
        <div class="fin-kpi-accent" style="background:{sol_color}"></div>
        <div class="fin-kpi-label"><span class="fin-kpi-label-dot" style="background:{sol_color}"></span>Solde caisse</div>
        <div class="fin-kpi-value" style="color:{sol_color}">{sol_sign}{kpi_sol:,.0f}</div>
        <div class="fin-kpi-sub">MAD net</div>
      </div>
      <div class="fin-kpi-cell">
        <div class="fin-kpi-accent" style="background:#1565C0"></div>
        <div class="fin-kpi-label"><span class="fin-kpi-label-dot" style="background:#1565C0"></span>Total encaissé</div>
        <div class="fin-kpi-value" style="color:#1565C0">{kpi_enc:,.0f}</div>
        <div class="fin-kpi-sub">MAD reçu</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with ftab1:
        ...
        # Stats rapides
        nb_df = len(df_df) if not df_df.empty else 0
        en_retard_df = len(df_df[df_df["Statut"]=="En retard"]) if not df_df.empty else 0
        taux_moy = df_df["Taux interet (%)"].mean() if not df_df.empty else 0

        st.markdown(f"""
        <div class="fin-row-stat">
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Dettes actives</div>
            <div class="fin-stat-pill-value">{nb_df}</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">En retard</div>
            <div class="fin-stat-pill-value" style="color:{'#B03A2E' if en_retard_df > 0 else '#1C1C1C'}">{en_retard_df}</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Taux moyen</div>
            <div class="fin-stat-pill-value">{taux_moy:.1f}%</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Déjà remboursé</div>
            <div class="fin-stat-pill-value" style="color:#2D7A3A">{df_df['Montant rembourse (MAD)'].sum() if not df_df.empty else 0:,.0f} MAD</div>
          </div>
        </div>
        """, unsafe_allow_html=True)

        # Formulaire ajout
        with st.expander("➕  Nouvelle dette financière", expanded=False):
            st.markdown('<div class="fin-form-wrap"><div class="fin-form-title">📝 Saisie de la dette</div>', unsafe_allow_html=True)
            fc1, fc2, fc3 = st.columns(3, gap="large")
            with fc1:
                df_date = st.date_input("Date", datetime.now(), key="df_date")
                df_crea = st.text_input("Créancier / Banque", key="df_crea", placeholder="Ex: BMCE, Ali Hassan…")
                df_type = st.selectbox("Type de dette", ["Emprunt bancaire","Crédit fournisseur","Prêt personnel","Autre"], key="df_type")
            with fc2:
                df_mi   = st.number_input("Montant initial (MAD)", min_value=0.0, step=100.0, key="df_mi")
                df_mr   = st.number_input("Déjà remboursé (MAD)", min_value=0.0, step=100.0, key="df_mr")
                df_taux = st.number_input("Taux intérêt (%)", min_value=0.0, step=0.1, key="df_taux")
            with fc3:
                df_ech  = st.date_input("Date échéance", key="df_ech")
                df_stat = st.selectbox("Statut", ["En cours","Remboursé","En retard","Renégocié"], key="df_stat")
            ...
            if st.button("Enregistrer la dette", key="btn_df_save"):
                ...
                try:
                    ...
                    st.success("Dette financière enregistrée.")
        ...
        if not df_df.empty:
            ...
            st.markdown(f'<div class="fin-table-label">{len(df_df_d)} dette(s)</div>', unsafe_allow_html=True)
            st.dataframe(df_df_d, hide_index=True, use_container_width=True)

            if len(df_df_d) > 0:
                fig = go.Figure()
                fig.add_trace(go.Bar(name="Initial", x=df_df_d["Creancier"], y=df_df_d["Montant initial (MAD)"],
                    marker_color="#E8B4B0", marker_line_width=0))
                fig.add_trace(go.Bar(name="Remboursé", x=df_df_d["Creancier"], y=df_df_d["Montant rembourse (MAD)"],
                    marker_color="#2D7A3A", marker_line_width=0))
                fig.update_layout(**PL, barmode="overlay",
                    legend=dict(orientation="h", y=-0.2, x=0.5, xanchor="center"),
                    xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor="#F0EDE5", tickformat=",.0f"))
                st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Aucune dette financière enregistrée.")

        # Remboursement rapide
        if not df_df.empty:
            st.markdown('<div class="fin-action-row"><div class="fin-action-title">Enregistrer un remboursement</div>', unsafe_allow_html=True)
            ra1, ra2 = st.columns([2,1])
            with ra1:
                creas = df_df["Creancier"].dropna().unique().tolist()
                rc = st.selectbox("Créancier", ["— sélectionner —"] + creas, key="rc_crea")
            with ra2:
                rm = st.number_input("Montant (MAD)", min_value=0.0, step=100.0, key="rc_montant", label_visibility="visible")
            st.markdown('</div>', unsafe_allow_html=True)
            if rc != "— sélectionner —" and st.button("Enregistrer le remboursement", key="btn_rc"):
                ...
                try:
                    ...
                    st.success(f"Remboursement de {rm:,.0f} MAD enregistré.")
                    clear_data_cache(); st.rerun()
                except Exception as e: st.error(f"Erreur : {e}")
    ...
    with ftab2:
        ...
        nb_dfo = len(df_dfo) if not df_dfo.empty else 0
        a_payer_dfo = len(df_dfo[df_dfo["Statut"]=="A payer"]) if not df_dfo.empty else 0
        solde_dfo = (df_dfo["Montant du (MAD)"] - df_dfo["Montant paye (MAD)"]).clip(lower=0).sum() if not df_dfo.empty else 0

        st.markdown(f"""
        <div class="fin-row-stat">
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Fournisseurs</div>
            <div class="fin-stat-pill-value">{nb_dfo}</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">À payer</div>
            <div class="fin-stat-pill-value" style="color:{'#7A4100' if a_payer_dfo > 0 else '#1C1C1C'}">{a_payer_dfo}</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Solde restant</div>
            <div class="fin-stat-pill-value" style="color:#7A4100">{solde_dfo:,.0f} MAD</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Déjà payé</div>
            <div class="fin-stat-pill-value" style="color:#2D7A3A">{df_dfo['Montant paye (MAD)'].sum() if not df_dfo.empty else 0:,.0f} MAD</div>
          </div>
        </div>
        """, unsafe_allow_html=True)

        with st.expander("➕  Nouvelle dette fournisseur", expanded=False):
            st.markdown('<div class="fin-form-wrap"><div class="fin-form-title">📝 Saisie de la dette fournisseur</div>', unsafe_allow_html=True)
            ...
            with ff1:
                ...
                dfo_lot  = st.selectbox("Lot associé", ["—"] + lots_list, key="dfo_lot")
            with ff2:
                dfo_desc = st.text_input("Description", key="dfo_desc")
                dfo_du   = st.number_input("Montant dû (MAD)", min_value=0.0, step=100.0, key="dfo_du")
                dfo_pay  = st.number_input("Déjà payé (MAD)", min_value=0.0, step=100.0, key="dfo_pay")
            with ff3:
                dfo_ech  = st.date_input("Date échéance", key="dfo_ech")
                dfo_stat = st.selectbox("Statut", ["A payer","Partiellement payé","Soldé","En litige"], key="dfo_stat")
            ...
            if st.button("Enregistrer la dette fournisseur", key="btn_dfo_save"):
                ...
                try:
                    ...
                    st.success("Dette fournisseur enregistrée.")
                    clear_data_cache(); st.rerun()
                except Exception as e: st.error(f"Erreur : {e}")
        ...
        if not df_dfo.empty:
            ...
            st.markdown(f'<div class="fin-table-label">{len(df_dfo_d)} dette(s) fournisseur</div>', unsafe_allow_html=True)
            ...
            st.markdown('<div class="fin-table-label">Résumé par fournisseur</div>', unsafe_allow_html=True)
            res = df_dfo.groupby("Fournisseur").apply(lambda x: pd.Series({
                "Total dû": x["Montant du (MAD)"].sum(),
                "Total payé": x["Montant paye (MAD)"].sum(),
                "Restant": (x["Montant du (MAD)"] - x["Montant paye (MAD)"]).clip(lower=0).sum(),
            }), include_groups=False).reset_index()
            st.dataframe(res, hide_index=True, use_container_width=True)
        else:
            st.info("Aucune dette fournisseur enregistrée.")

        # Paiement rapide
        if not df_dfo.empty:
            st.markdown('<div class="fin-action-row"><div class="fin-action-title">Enregistrer un paiement fournisseur</div>', unsafe_allow_html=True)
            pa1, pa2 = st.columns([2,1])
            with pa1:
                fours = df_dfo["Fournisseur"].dropna().unique().tolist()
                pf = st.selectbox("Fournisseur", ["— sélectionner —"] + fours, key="pf_four")
            with pa2:
                pm = st.number_input("Montant payé (MAD)", min_value=0.0, step=100.0, key="pf_montant")
            st.markdown('</div>', unsafe_allow_html=True)
            if pf != "— sélectionner —" and st.button("Enregistrer le paiement", key="btn_pf"):
                ...
                for i in df_dfo[mask].index:
                    ...
                    if r <= 0: df_dfo.at[i,"Statut"] = "Soldé"
                    elif df_dfo.at[i,"Montant paye (MAD)"] > 0: df_dfo.at[i,"Statut"] = "Partiellement payé"
                try:
                    save_sheet(df_dfo, "Dette fournisseur")
                    st.success(f"Paiement de {pm:,.0f} MAD enregistré.")
                    clear_data_cache(); st.rerun()
                except Exception as e: st.error(f"Erreur : {e}")
    ...
    with ftab3:
        ...
        c_sign = "+" if c_sol >= 0 else ""
        nb_ops = len(df_ca) if not df_ca.empty else 0

        st.markdown(f"""
        <div class="fin-row-stat">
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Entrées</div>
            <div class="fin-stat-pill-value" style="color:#2D7A3A">{c_in:,.0f} MAD</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Sorties</div>
            <div class="fin-stat-pill-value" style="color:#B03A2E">{c_out:,.0f} MAD</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Solde net</div>
            <div class="fin-stat-pill-value" style="color:{c_col}">{c_sign}{c_sol:,.0f} MAD</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Opérations</div>
            <div class="fin-stat-pill-value">{nb_ops}</div>
          </div>
        </div>
        """, unsafe_allow_html=True)

        with st.expander("➕  Nouvelle opération de caisse", expanded=False):
            st.markdown('<div class="fin-form-wrap"><div class="fin-form-title">📝 Saisie de l\'opération</div>', unsafe_allow_html=True)
            cc1, cc2, cc3 = st.columns(3, gap="large")
            with cc1:
                ca_date = st.date_input("Date", datetime.now(), key="ca_date")
                ca_type = st.selectbox("Type opération", ["ENTREE","SORTIE"], key="ca_type")
                ca_cat  = st.selectbox("Catégorie", ["Vente marchandise","Achat marchandise","Loyer","Salaire",
            ...
            with cc2:
                ...
                ca_mode = st.selectbox("Mode", ["Espèces","Virement","Chèque","Mobile Payment","Autre"], key="ca_mode")
            with cc3:
                ca_lot  = st.selectbox("Lot associé (optionnel)", ["—"] + lots_list, key="ca_lot")
                ca_rem  = st.text_input("Remarque", key="ca_rem")
            st.markdown('</div>', unsafe_allow_html=True)
            if st.button("Enregistrer l'opération", key="btn_ca_save"):
                ...
                try:
                    ...
                    st.success("Opération de caisse enregistrée.")
                    clear_data_cache(); st.rerun()
                except Exception as e: st.error(f"Erreur : {e}")
        ...
        if not df_ca.empty:
            ...
            with ca2:
                ...
                fca_c = st.selectbox("Catégorie", cats, key="fca_c")
            with ca3:
            ...
            st.markdown(f'<div class="fin-table-label">{len(df_ca_d)} opération(s)</div>', unsafe_allow_html=True)
            st.dataframe(df_ca_d, hide_index=True, use_container_width=True)

            # Graphique evolution
            st.markdown('<div class="fin-divider"></div>', unsafe_allow_html=True)
            st.markdown('<div class="fin-table-label">Évolution de la caisse</div>', unsafe_allow_html=True)
            df_ca_t = df_ca.copy()
            ...
            df_ca_t["Solde cumulé"] = df_ca_t["Flux"].cumsum()
            if not df_ca_t.empty:
                fig_ca = go.Figure()
                fig_ca.add_trace(go.Scatter(x=df_ca_t["Date"], y=df_ca_t["Solde cumulé"],
                    mode="lines+markers", name="Solde cumulé",
                    line=dict(color="#1565C0", width=2),
                    fill="tozeroy", fillcolor="rgba(21,101,192,0.06)"))
                fig_ca.add_trace(go.Bar(x=df_ca_t["Date"], y=df_ca_t["Flux"], name="Flux",
                    marker_color=["#2D7A3A" if v >= 0 else "#B03A2E" for v in df_ca_t["Flux"]], opacity=0.45))
                fig_ca.update_layout(**PL,
                    legend=dict(orientation="h", y=-0.2, x=0.5, xanchor="center"),
                    xaxis=dict(showgrid=False),
                    yaxis=dict(showgrid=True, gridcolor="#F0EDE5", tickformat=",.0f"))
                st.plotly_chart(fig_ca, use_container_width=True)

            st.markdown('<div class="fin-divider"></div>', unsafe_allow_html=True)
            st.markdown('<div class="fin-table-label">Résumé par catégorie</div>', unsafe_allow_html=True)
            ...
            st.info("Aucune opération de caisse enregistrée.")
    ...
    with ftab4:
        ...
        nb_enc   = len(df_enc) if not df_enc.empty else 0

        st.markdown(f"""
        <div class="fin-row-stat">
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Total encaissé</div>
            <div class="fin-stat-pill-value" style="color:#1565C0">{enc_tot:,.0f} MAD</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Confirmé reçu</div>
            <div class="fin-stat-pill-value" style="color:#2D7A3A">{enc_recu:,.0f} MAD</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">En attente</div>
            <div class="fin-stat-pill-value" style="color:#7A4100">{enc_att:,.0f} MAD</div>
          </div>
          <div class="fin-stat-pill">
            <div class="fin-stat-pill-label">Encaissements</div>
            <div class="fin-stat-pill-value">{nb_enc}</div>
          </div>
        </div>
        """, unsafe_allow_html=True)

        with st.expander("➕  Enregistrer un encaissement", expanded=False):
            st.markdown('<div class="fin-form-wrap"><div class="fin-form-title">📝 Saisie de l\'encaissement</div>', unsafe_allow_html=True)
            ...
            with ec1:
                ...
                en_lot  = st.selectbox("Lot associé", ["—"] + lots_list, key="en_lot")
            with ec2:
                en_desc = st.text_input("Description", key="en_desc")
                en_mont = st.number_input("Montant (MAD)", min_value=0.0, step=10.0, key="en_mont")
                en_mode = st.selectbox("Mode de paiement", ["Espèces","Virement","Chèque","Mobile Payment","Carte bancaire","Autre"], key="en_mode")
            with ec3:
                ...
                en_stat = st.selectbox("Statut", ["Recu","En attente","Partiellement reçu","Annulé"], key="en_stat")
            ...
            if st.button("Enregistrer l'encaissement", key="btn_en_save"):
                ...
                try:
                    ...
                    st.success("Encaissement enregistré.")
                    clear_data_cache(); st.rerun()
                except Exception as e: st.error(f"Erreur : {e}")
        ...
        if not df_enc.empty:
            ...
            st.markdown(f'<div class="fin-table-label">{len(df_enc_d)} encaissement(s)</div>', unsafe_allow_html=True)
            ...
            if not enc_lot.empty:
                st.markdown('<div class="fin-divider"></div>', unsafe_allow_html=True)
                st.markdown('<div class="fin-table-label">Encaissement par lot</div>', unsafe_allow_html=True)
                fig_enc = go.Figure(go.Bar(
                    x=enc_lot["Lot"], y=enc_lot["Montant (MAD)"],
                    marker_color="#1565C0", marker_line_width=0,
                    hovertemplate="%{x}<br>%{y:,.0f} MAD<extra></extra>"))
                fig_enc.update_layout(**PL,
                    xaxis=dict(showgrid=False),
                    yaxis=dict(showgrid=True, gridcolor="#F0EDE5", tickformat=",.0f"))
                st.plotly_chart(fig_enc, use_container_width=True)

            st.markdown('<div class="fin-divider"></div>', unsafe_allow_html=True)
            st.markdown('<div class="fin-table-label">Résumé par client / payeur</div>', unsafe_allow_html=True)
            res_enc = df_enc.groupby("Payeur").agg(
                Total=("Montant (MAD)", "sum"),
                Nb=("Montant (MAD)", "count")
