...
[data-testid="stDataEditor"] [role="option"],[data-testid="stDataEditor"] [role="listbox"] li,[data-testid="stDataEditor"] [role="listbox"] [role="option"]{color:#1C1C1C !important;background:#FFFFFF !important;font-family:'DM Sans',sans-serif !important;font-size:0.88rem !important}
[data-testid="stDataEditor"] [role="listbox"]{background:#FFFFFF !important;border:1px solid #E0DDD5 !important;border-radius:8px !important;box-shadow:0 4px 16px rgba(28,28,28,0.12) !important}
div[class*="gdg-"][role="listbox"],div[class*="gdg-"] [role="listbox"],div[class*="gdg-"] [role="option"]{color:#1C1C1C !important;background:#FFFFFF !important;font-family:'DM Sans',sans-serif !important}
...
#mahal-drawer{position:fixed;top:0;right:-340px;width:300px;height:100vh;background:#FFFFFF;border-left:1px solid #E0DDD5;z-index:9999;transition:right 0.35s cubic-bezier(0.4,0,0.2,1);display:flex;flex-direction:column;overflow:hidden;box-shadow:-8px 0 32px rgba(28,28,28,0.08)}
#mahal-drawer.open{right:0}
.drawer-header{padding:2rem 1.8rem 1.4rem 1.8rem;border-bottom:1px solid #F0EDE5;display:flex;justify-content:space-between;align-items:flex-end;flex-shrink:0}
...
@media screen and (max-width:768px){
  .block-container{padding:0.9rem 0.9rem 5.5rem 0.9rem !important;max-width:100% !important}
  .auth-left{display:none !important}
  .page-title{font-size:1.7rem !important}
  .page-subtitle{font-size:0.68rem !important;margin-bottom:0.9rem !important}
  .metric-row{display:grid !important;grid-template-columns:1fr 1fr !important;gap:0.55rem !important;margin-bottom:1.1rem !important}
  .metric-card{padding:0.9rem 1rem !important;border-radius:12px !important}
  .metric-value{font-size:1.15rem !important}
  .section-title{font-size:1.05rem !important;margin-top:1.1rem !important;margin-bottom:0.7rem !important}
  #mahal-drawer{width:86vw !important;right:-86vw !important}
  .topbar{flex-wrap:wrap;gap:0.5rem}
  .topbar-user{display:none}
  .active-tab-pill{margin-left:0}
  .active-tab-pill span:nth-child(2){max-width:32vw;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:inline-block}
  .stButton>button{min-height:46px !important;width:100% !important;padding:0.7rem 1.2rem !important;border-radius:10px !important}
  .stTextInput>div>div>input,.stNumberInput>div>div>input,.stDateInput>div>div>input,.stSelectbox>div>div>div{min-height:46px !important;font-size:1rem !important}
  ...
  .stTabs [data-baseweb="tab"]{padding:0.6rem 0.9rem !important;font-size:0.82rem !important}
  .stDataFrame,[data-testid="stDataEditor"]{font-size:0.8rem !important}
  .edit-card{padding:1.1rem !important;border-radius:14px !important}
  .user-card{padding:0.85rem 1rem !important}
}
@media screen and (max-width:480px){
  .block-container{padding:0.6rem 0.55rem 5.5rem 0.55rem !important}
...
# ─── PDF Rapport multi-lots (historique) ───────────────────────────────────────
def generate_lots_report_pdf(transactions_df, lots_selected, meta: dict) -> bytes:
    """Génère un rapport PDF multi-pages avec l'historique des lots sélectionnés.
    transactions_df: DataFrame déjà filtré (lots + période + type) contenant les colonnes usuelles.
    lots_selected: liste des lots inclus dans le rapport (ordre d'affichage).
    meta: dict avec 'titre', 'periode', 'genere_par'.
    """
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=A4,
        rightMargin=16*mm, leftMargin=16*mm, topMargin=15*mm, bottomMargin=15*mm)
    ...
    title_style = ParagraphStyle('RTitle', parent=styles['Normal'],
        fontName='Helvetica-Bold', fontSize=24, textColor=dark, leading=28)
    sub_style = ParagraphStyle('RSub', parent=styles['Normal'],
        fontName='Helvetica', fontSize=9, textColor=grey, leading=13)
    h2_style = ParagraphStyle('RH2', parent=styles['Normal'],
        fontName='Helvetica-Bold', fontSize=13, textColor=dark, leading=16, spaceAfter=2*mm)
    small_style = ParagraphStyle('RSmall', parent=styles['Normal'],
        fontName='Helvetica', fontSize=7.5, textColor=dark, leading=10)
    small_bold = ParagraphStyle('RSmallB', parent=styles['Normal'],
        fontName='Helvetica-Bold', fontSize=7.5, textColor=colors.white, leading=10)
    footer_style = ParagraphStyle('RFooter', parent=styles['Normal'],
        fontName='Helvetica', fontSize=7.5, textColor=grey, alignment=TA_CENTER, leading=11)

    story = []

    # ── Page de garde / en-tête général ──
    header_data = [[
        Paragraph("MAHAL", title_style),
        Paragraph(meta.get('genere_le', datetime.now().strftime('%d/%m/%Y à %H:%M')),
            ParagraphStyle('RDate', parent=styles['Normal'], fontName='Helvetica', fontSize=9,
                           textColor=grey, alignment=TA_RIGHT))
    ]]
    ht = Table(header_data, colWidths=[110*mm, 60*mm])
    ht.setStyle(TableStyle([('VALIGN',(0,0),(-1,-1),'MIDDLE'), ('BOTTOMPADDING',(0,0),(-1,-1),3*mm)]))
    story.append(ht)
    story.append(Paragraph(meta.get('titre', 'Rapport de gestion'), sub_style))
    story.append(HRFlowable(width="100%", thickness=1, color=border, spaceAfter=5*mm))

    # Bandeau de synthèse (résumé global)
    ta_tot = transactions_df.loc[transactions_df['Type (Achat/Vente/Dépense)']=='ACHAT','Montant (MAD)'].sum() if not transactions_df.empty else 0
    tv_tot = transactions_df.loc[transactions_df['Type (Achat/Vente/Dépense)']=='VENTE','Montant (MAD)'].sum() if not transactions_df.empty else 0
    td_tot = transactions_df.loc[transactions_df['Type (Achat/Vente/Dépense)']=='DÉPENSE','Montant (MAD)'].sum() if not transactions_df.empty else 0
    rn_tot = tv_tot - (ta_tot + td_tot)
    rn_color = green if rn_tot >= 0 else red

    kpi_lbl = ParagraphStyle('KLbl', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=7.5, textColor=grey, leading=10)
    kpi_val = ParagraphStyle('KVal', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=13, textColor=dark, leading=16)

    def kpi_cell(label, value, color=dark):
        v_style = ParagraphStyle('KValC', parent=kpi_val, textColor=color)
        return [Paragraph(label, kpi_lbl), Paragraph(f"{value:,.0f} MAD", v_style)]

    kpi_table_data = [[
        Paragraph("LOTS INCLUS", kpi_lbl), Paragraph("TOTAL ACHATS", kpi_lbl),
        Paragraph("TOTAL VENTES", kpi_lbl), Paragraph("TOTAL DÉPENSES", kpi_lbl), Paragraph("RÉSULTAT NET", kpi_lbl)
    ], [
        Paragraph(str(len(lots_selected)), kpi_val),
        Paragraph(f"{ta_tot:,.0f} MAD", ParagraphStyle('KV1', parent=kpi_val)),
        Paragraph(f"{tv_tot:,.0f} MAD", ParagraphStyle('KV2', parent=kpi_val, textColor=green)),
        Paragraph(f"{td_tot:,.0f} MAD", ParagraphStyle('KV3', parent=kpi_val)),
        Paragraph(f"{rn_tot:+,.0f} MAD", ParagraphStyle('KV4', parent=kpi_val, textColor=rn_color)),
    ]]
    kpi_t = Table(kpi_table_data, colWidths=[34*mm]*5)
    ...
    if meta.get('periode'):
        story.append(Paragraph(f"<b>Période :</b> {meta['periode']}", small_style))
        story.append(Spacer(1, 2*mm))
    if meta.get('genere_par'):
        story.append(Paragraph(f"<b>Généré par :</b> {meta['genere_par']}", small_style))
        story.append(Spacer(1, 5*mm))

    # ── Détail par lot ──
    col_defs = [
        ("Date", 18*mm), ("Type", 16*mm), ("Personne", 26*mm), ("Description", 42*mm),
        ("Montant (MAD)", 22*mm), ("Qté", 10*mm), ("Mode paiement", 22*mm), ("Remarque", 22*mm)
    ]

    for lot in lots_selected:
        t_lot = transactions_df[transactions_df['Lot'].astype(str) == str(lot)].copy()
        if 'Date' in t_lot.columns:
            t_lot = t_lot.sort_values('Date', ascending=False)

        a_l = t_lot.loc[t_lot['Type (Achat/Vente/Dépense)']=='ACHAT','Montant (MAD)'].sum() if not t_lot.empty else 0
        v_l = t_lot.loc[t_lot['Type (Achat/Vente/Dépense)']=='VENTE','Montant (MAD)'].sum() if not t_lot.empty else 0
        d_l = t_lot.loc[t_lot['Type (Achat/Vente/Dépense)']=='DÉPENSE','Montant (MAD)'].sum() if not t_lot.empty else 0
        r_l = v_l - (a_l + d_l)
        r_color = green if r_l >= 0 else red

        story.append(Paragraph(f"Lot — {lot}", h2_style))
        r_hex = "#2D7A3A" if r_l >= 0 else "#B03A2E"
        result_style = ParagraphStyle('RResult', parent=small_style, fontName='Helvetica-Bold', textColor=colors.HexColor(r_hex))
        sum_data = [[
            Paragraph(f"Achats : {a_l:,.0f} MAD", small_style),
            Paragraph(f"Ventes : {v_l:,.0f} MAD", small_style),
            Paragraph(f"Dépenses : {d_l:,.0f} MAD", small_style),
            Paragraph(f"Résultat : {r_l:+,.0f} MAD", result_style),
        ]]
        sum_t = Table(sum_data, colWidths=[42*mm, 42*mm, 42*mm, 42*mm])
        ...
        if t_lot.empty:
            story.append(Paragraph("Aucune transaction enregistrée pour ce lot sur la période sélectionnée.", small_style))
            story.append(Spacer(1, 6*mm))
            continue

        header_row = [Paragraph(c[0], small_bold) for c in col_defs]
        table_rows = [header_row]
        type_row_colors = []
        for _, r in t_lot.iterrows():
            typ = str(r.get('Type (Achat/Vente/Dépense)',''))
            try:
                montant_txt = f"{float(r.get('Montant (MAD)',0)):,.0f}"
            except Exception:
                montant_txt = "0"
            row = [
                Paragraph(str(r.get('Date',''))[:10], small_style),
                Paragraph(typ, small_style),
                Paragraph(str(r.get('Personne',''))[:22], small_style),
                Paragraph(str(r.get('Description',''))[:60], small_style),
                Paragraph(montant_txt, small_style),
                Paragraph(str(r.get('Quantité (pièces)','')), small_style),
                Paragraph(str(r.get('Mode de paiement',''))[:20], small_style),
                Paragraph(str(r.get('Remarque',''))[:20], small_style),
            ]
            table_rows.append(row)
            if typ == 'ACHAT': type_row_colors.append(colors.HexColor("#EEF2FB"))
            elif typ == 'VENTE': type_row_colors.append(colors.HexColor("#EEF7EE"))
            elif typ == 'DÉPENSE': type_row_colors.append(colors.HexColor("#FBF2E9"))
            else: type_row_colors.append(colors.white)
        ...
        for i, c in enumerate(type_row_colors, start=1):
...
if is_admin:
    nav_items = [
        ...
        {"key": "rapports_pdf",         "label": "Rapports PDF",         "icon": "⬇", "section": "Export"},
...
for _it in _nav_items_for_drawer:
    ...
    _dih += (
        '<button data-navkey="' + _it["key"] + '" style="display:flex;align-items:center;gap:0.9rem;padding:0.85rem 1.8rem;cursor:pointer;border:none;background:' + _abg + ';width:100%;text-align:left;font-family:DM Sans,sans-serif;font-size:0.9rem;letter-spacing:0.01em;position:relative;font-weight:' + _afw + ';min-height:48px">'
        + _bar
        + '<span style="width:32px;height:32px;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;font-size:0.95rem;flex-shrink:0;background:' + _ibg + ';color:' + _ico + '">' + _it["icon"] + '</span>'
...
_html_parts = [
    ...
    '  var bmenu=pdoc.getElementById("__mahal_bottom_menu");',
    ...
    '  if(bb)bb.addEventListener("click",function(e){var t=e.target;while(t&&t!==bb){if(t.id==="__mahal_bottom_menu"){openDrawer();return;}if(t.dataset&&t.dataset.navkey){navTo(t.dataset.navkey);return;}t=t.parentElement;}});',
    '},50);',
...
# ═══════════════════════════════════════════════════════════════════════════════
# RAPPORTS PDF — Sélection de lots + export historique en PDF
# ═══════════════════════════════════════════════════════════════════════════════
def render_rapports_pdf_page():
    st.markdown('<div class="section-title">Rapports PDF</div>', unsafe_allow_html=True)
    st.caption("Choisissez un ou plusieurs lots et générez un PDF propre à envoyer à votre client — historique complet, achats, ventes et dépenses inclus.")

    lots_dispo = lots_existants if not is_admin else sorted(transactions_all['Lot'].dropna().astype(str).unique().tolist())
    if not lots_dispo:
        st.info("Aucun lot disponible pour le moment.")
        return

    rc1, rc2 = st.columns([2, 1], gap="large")
    with rc1:
        mode_sel = st.radio("Lots à inclure", ["Sélection manuelle", "Tous les lots visibles"],
                             horizontal=True, key="rap_mode")
        if mode_sel == "Tous les lots visibles":
            lots_choisis = lots_dispo
            st.multiselect("Lots inclus", options=lots_dispo, default=lots_dispo, disabled=True, key="rap_lots_disabled")
        else:
            lots_choisis = st.multiselect("Sélectionner les lots", options=lots_dispo, key="rap_lots")
    with rc2:
        type_filtre = st.selectbox("Type de transaction", ["Tous","ACHAT","VENTE","DÉPENSE"], key="rap_type")
    ...
    if lots_choisis:
        df_export = df_export[df_export['Lot'].astype(str).isin(lots_choisis)]
    else:
        df_export = df_export.iloc[0:0]

    if type_filtre != "Tous":
        df_export = df_export[df_export['Type (Achat/Vente/Dépense)'] == type_filtre]

    periode_txt = ""
    if date_debut:
        d_parsed = pd.to_datetime(df_export['Date'], errors='coerce')
        df_export = df_export[d_parsed >= pd.Timestamp(date_debut)]
        periode_txt += f"du {date_debut.strftime('%d/%m/%Y')} "
    if date_fin:
        d_parsed = pd.to_datetime(df_export['Date'], errors='coerce')
        df_export = df_export[d_parsed <= pd.Timestamp(date_fin)]
        periode_txt += f"au {date_fin.strftime('%d/%m/%Y')}"
    if not periode_txt:
        periode_txt = "toutes dates confondues"

    st.markdown(f'<div class="info-count">{len(df_export)} transaction(s) sur {len(lots_choisis)} lot(s) sélectionné(s)</div>', unsafe_allow_html=True)

    if lots_choisis:
        aperc = compute_suivi_lot(df_export)
        if not aperc.empty:
            st.dataframe(aperc, hide_index=True, use_container_width=True)

    st.markdown("---")
    nom_client = st.text_input("Nom du client / destinataire (optionnel, apparaît sur le rapport)", key="rap_client", placeholder="Ex: M. Alaoui")

    gen_col1, gen_col2 = st.columns([1, 2])
    with gen_col1:
        generer = st.button("📄 Générer le PDF", key="btn_gen_rapport", use_container_width=True)

    if generer:
        if not lots_choisis:
            st.error("Sélectionne au moins un lot.")
        else:
            try:
                titre = f"Rapport de gestion — {', '.join(lots_choisis) if len(lots_choisis) <= 4 else str(len(lots_choisis)) + ' lots'}"
                if nom_client.strip():
                    titre += f" — pour {sanitize_text(nom_client)}"
                meta = {
                    "titre": titre,
                    "periode": periode_txt,
                    "genere_par": username.upper(),
                }
                pdf_bytes = generate_lots_report_pdf(df_export, lots_choisis, meta)
                fname = f"rapport_mahal_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
...
def render_page():
    ...
    # ── RAPPORTS PDF ───────────────────────────────────────────────────────────
    elif page == "rapports_pdf":
        render_rapports_pdf_page()
